...
echo '---'
echo 'Today (click to edit) | color=green bash=pomodoro param1=edit' # terminal=false
echo '---'
pomodoro today | ruby -e 'first_line, *rest = ARGF.readlines.map(&:chomp); puts "#{first_line} | color=red", rest'

echo '---'
echo 'Active projects (click to edit) | color=green bash=atom param1=~/.config/pomodoro/schedules/projects.rb'
pomodoro show-projects | sed 's/^/--/'
echo '---'

for task_list in ~/tasks.todo ~/.bitbar/tasks.todo ~/Dropbox/Projects/*/*/tasks.todo; do
  project_name=$(basename "$(dirname $task_list)")
  test "$project_name" = 'botanicus' && project_name='dotfiles'
  echo "$project_name (click to edit) | bash=atom param1='${task_list}'" # TODO: terminal=false
  cat $task_list | sed -E 's/^(.+)$/--\1 | color=black/'
done
